...
class MassflowController(Drivable):
    ...
    ramp    = Parameter("desired ramp speed for gas flow of gas",FloatRange(minval=0,maxval=200),default = 0,unit = "ml/min^2" )
    gastype = Parameter("chemical formula of gas type handled by flow controller",StringType(maxchars=50,minchars=0),default = "Null")
    ...
    def __sim(self):
        ...
        while not self._stopflag:
            ...
            if self.regulationFlow != self.target:
                ...
                try:
                    self.setpoint = round(self.setpoint + clamp(self.target - self.setpoint, -maxdelta, maxdelta), 3)
            ...
            self.regulationFlow = round(regulation, 3)
